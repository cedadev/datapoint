    ...
    def __getitem__(self, index) -> DataPointItem:
        ...
        if isinstance(index, str):
            if index not in self._item_set:
                logger.warning(f'Item "{index}" not present in the set of items.')
                return None
        ...
        **kwargs,
        ...
        if mode != "xarray":
            raise NotImplementedError(
                "No other modes currently implemented than default xarray"
            )
        if combine:
...
class DataPointClient(UIMixin):
    ...
    def _collection_search(self, q: str) -> list:
        ...
        if self._api_collection_search:
            ...
            raise NotImplementedError(f'Blind Collection Search not implemented on {self._url}')
