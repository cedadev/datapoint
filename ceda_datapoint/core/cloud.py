...
class DataPointCloudProduct(BasicAsset):
    ...
    def open_asset(
        self, local_only: bool = False, prepare_data: bool = True, **kwargs
    ) -> Any:
        """
    ...
    def open_dataset(
        ...
        **kwargs,
        ...
        if mode != "xarray":
            raise NotImplementedError(
                "Non-xarray opening is not implemented for cloud products yet."
            )

        if not self._cloud_format:
            raise ValueError("No cloud format given for this dataset")

        if self.href is None:
            raise ValueError('Cloud assets with no "href" are not supported')
        ...
        try:
            ...
            raise FileNotFoundError(
                "The requested resource could not be located: " f"{self.href}"
            )
    ...
    def _open_kerchunk(
        ...
        **kwargs,
        ...
        href = self.href
        mapper_kwargs = (
            self._mapper.get("mapper_kwargs", self._asset_stac) or mapper_kwargs
        )
    ...
    def _open_cfa(
        ...
        **kwargs,
        ...
        open_xarray_kwargs = (
            self._mapper.get("open_xarray_kwargs", self._asset_stac) or {}
        ) | kwargs

        return xr.open_dataset(
            href, engine="CFA", cfa_options=cfa_options, **open_xarray_kwargs
        )
    ...
    def _open_zarr(
        ...
        **kwargs,
        ...
        open_zarr_kwargs = (
            self._mapper.get("open_zarr_kwargs", self._asset_stac) or {}
        ) | kwargs
    ...
    def _open_cog(
        ...
        **kwargs,
        ...
        open_cog_kwargs = (
            self._mapper.get("open_cog_kwargs", self._asset_stac) or {}
        ) | kwargs

        return rxr.open_rasterio(self.href, **open_cog_kwargs)

    def _prepare_dataset(
        self, ds: xr.Dataset, prepare_data: bool = True
...
class DataPointCluster(UIMixin):
    ...
    def __getitem__(self, index):
        ...
        if index not in self._products:
            raise IndexError(f'"{index}" not found in available products.')
    ...
    def open_dataset(
        ...
        **kwargs,
        ...
        if id not in self._products:
            logger.warning(f'"{id}" not found in available datasets.')
            return None
    ...
    def open_datasets(self):
        raise NotImplementedError('"Combine" feature has not yet been implemented')
